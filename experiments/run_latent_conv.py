...
from experiments.utils import plot_reconstructions_conv, save_metrics
import torch
import os
...
os.makedirs("results/reconstructions/conv_ae", exist_ok=True)
os.makedirs("results/metrics/conv_ae", exist_ok=True)
...
for latent_dim in latent_dims:
    print(f"\nTreinando Conv AE com espaço latente = {latent_dim}")
    ...
    plot_reconstructions_conv(model, test_loader, device,
                         save_path=f"results/reconstructions/conv_ae_latent{latent_dim}.png")
    save_metrics(metrics, filename=f"results/metrics/conv_ae/conv_ae_latent{latent_dim}.csv")